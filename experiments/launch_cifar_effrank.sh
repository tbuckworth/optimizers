...
mkdir -p "$OUT"
EP=40
CAP=400   # basis cap (cost scales with this); effrank only PROJECTS onto round(effrank)<<cap
run () { # noise adaptive name
  echo "=== $3  (noise=$1 adaptive=$2 decay=0.999 rankcap=$CAP) ==="
  python3 -u run_cifar_noise.py --mode ours --noise "$1" --decay 0.999 --rank $CAP \
