...
ax.set_xscale("log"); ax.set_xticks(ranks); ax.set_xticklabels(ranks, fontsize=7.5)
ax.set_ylim(0, 6400)
...
ax.set_title("Rank → grokking speed: ≤4 unstable, 5–8 grok\n(variably), ~10 fastest — none beats AdamW", color=INK, fontsize=11)
...
# ---- Panel 3: active gradient directions over time for PURE AdamW (passive, measured not used) ----
ax = fig.add_subplot(gs[0, 2]); ax.set_facecolor(PANEL)
PASS = "/Users/titus/pyg/optimizers/results/parity_passive"
def passive(s): return json.load(open(os.path.join(PASS, f"adamw_passive_s{s}.json")))["metrics"]
ms = [passive(s) for s in (0, 1, 2)]
ep = [r["epoch"] for r in ms[0]]
er = np.array([[ (r.get("eff_rank") or 0) for r in m] for m in ms]).mean(0)
te = np.array([[ r["test_acc"] for r in m] for m in ms]).mean(0)
ax.plot(ep, er, "-", color=AMBER, lw=2.2, label="active dirs (eff. rank)")
gmean = int(np.mean(adamw))
ax.axvline(gmean, color=GREEN, ls="--", lw=1.4)
ax.text(gmean+80, max(er)*0.2, f"AdamW groks\n@~{gmean}", color=GREEN, fontsize=8.5)
ax.set_xlabel("epoch"); ax.set_ylabel("# active gradient directions  (effective rank)")
ax2 = ax.twinx(); ax2.set_facecolor("none")
ax2.plot(ep, te, color=BLUE, lw=1.5, alpha=0.8)
ax2.set_ylabel("test acc", color=BLUE); ax2.tick_params(axis="y", labelcolor=BLUE); ax2.set_ylim(0.4, 1.03)
ax.set_title("Pure AdamW (covariance measured, NOT filtered):\nlow during fit → broadens after grok → narrows", color=INK, fontsize=10.5)
ax.grid(True, color=GRID, lw=0.5, alpha=0.5)
ax.legend(loc="center right", facecolor=PANEL, edgecolor=GRID, fontsize=8, labelcolor=INK)
