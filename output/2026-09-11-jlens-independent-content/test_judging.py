...
SPEC = importlib.util.spec_from_file_location("independent_content_judging_fixture_module", SOURCE)
...
def fabricated():
    dataset = [{"id": ident, "topic": ident.split("-")[0],
                "prefix": f"These fabricated words describe sample {n} with enough plain tokens for this fixed prefix fixture today"} for n, ident in enumerate(j.IDS)]
...
class PureTests(unittest.TestCase):
    ...
    def test_exact_roster_allocation_swaps_and_anonymous_structure(self):
        self.assertEqual(20260915, j.SEED)
        self.assertEqual({"rater1": ("A", "B", "C", "A"),
                          "rater2": ("B", "C", "A", "B"),
                          "rater3": ("C", "A", "B", "C"),
                          "rater4": ("A", "B", "C", "A"),
                          "rater5": ("B", "C", "A", "B"),
                          "rater6": ("C", "A", "B", "C")}, j.ALLOCATION)
        self.assertEqual(dict(zip(j.RATERS, (1, 1, 1, 2, 2, 2))), j.COHORT)
        ...
        self.assertEqual("jlens_independent_content_private_map_v1", self.mapping["schema"])
        self.assertEqual(288, len(self.mapping["rows"]))
        seen, cells = set(), {}
        for row in self.mapping["rows"]:
            ...
            cells.setdefault((row["axis"], row["cohort"], row["pair_id"]), []).append(row)
        self.assertEqual(96, len(cells))
        for rows in cells.values():
            self.assertEqual(set(j.ARMS), {r["arm"] for r in rows})
            self.assertEqual(1, len({(r["first_id"], r["second_id"]) for r in rows}))
        for axis in j.AXES:
            for pair in self.pairs:
                one = cells[axis, 1, pair["id"]][0]
                two = cells[axis, 2, pair["id"]][0]
                self.assertEqual((one["first_id"], one["second_id"]),
                                 (two["second_id"], two["first_id"]))
        self.assertEqual({f"Q{i:03}" for i in range(1, 289)}, seen)
        self.assertEqual(24, len({r["block_id"] for r in self.mapping["rows"]}))
        for packet in self.public.values():
            self.assertEqual("jlens_independent_content_public_v1", packet["schema"])
            self.assertEqual(48, len(j._public_ids(packet)))
            serialized = json.dumps(packet)
            for forbidden in ('"axis"', '"arm"', '"score"', '"truth"', '"source_id"', '"pair_id"', '"cohort"'):
    ...
    def test_only_fixed_adjacent_within_topic_roster_allowed(self):
        expected_edges = [(f"{topic}-wiki-{i}", f"{topic}-wiki-{i+1}")
    ...
    def test_response_rejection_and_strict_json(self):
        ...
        good = {"schema": "jlens_independent_content_responses_v1", "packet_id": packet["packet_id"],
    ...
    def test_exact_ties_small_gaps_and_all_grading_reductions(self):
        ...
        self.assertEqual("jlens_independent_content_grades_v1", result["schema"])
        self.assertEqual(288, len(result["items"]))
        self.assertEqual(95.0, result["arms"]["A"]["credit"])
        self.assertEqual(1.0, result["arms"]["B"]["credit"])
        self.assertEqual(94, result["paired_differences"]["A_minus_B"]["total_credit"])
        for arm in j.ARMS:
            summary = result["arms"][arm]
            self.assertEqual(96, summary["total"])
            self.assertEqual(2, summary["exact_ties"])
            self.assertEqual(48, summary["constant_FIRST_credit"])
            self.assertEqual(48, summary["constant_SECOND_credit"])
            self.assertEqual(summary["credit"], sum(result["per_axis"][x][arm]["credit"] for x in j.AXES))
            for axis in j.AXES:
                self.assertEqual(24, result["per_axis"][axis][arm]["total"])
                self.assertEqual(12, result["per_axis"][axis][arm]["constant_FIRST_credit"])
                self.assertEqual(12, result["per_axis"][axis][arm]["constant_SECOND_credit"])
        tiny = [r for r in result["items"] if r["axis"] == "PC2" and r["pair_id"] == "P02"]
        self.assertEqual(6, len(tiny))
    ...
    def test_reader_reductions_and_agreement_use_prefix_not_position(self):
        result = j._grade_values(self.mapping, self.choices(), self.scores)
        for rater, row in result["per_reader"].items():
            self.assertEqual(j.COHORT[rater], row["cohort"])
            self.assertEqual(48, row["summary"]["total"])
            self.assertEqual(48, sum(v["total"] for v in row["per_axis"].values()))
            for ai, axis in enumerate(j.AXES):
                self.assertEqual(j.ALLOCATION[rater][ai], row["per_axis"][axis]["arm"])
                self.assertEqual(12, row["per_axis"][axis]["total"])
        # Always-FIRST C has perfect literal-string agreement but selects opposite texts.
        self.assertEqual({"agree": 0, "disagree": 48, "total": 48},
                         result["agreement"]["arms"]["C"])
        # Correct non-tie A responses use opposite labels and agree on the selected text.
        self.assertEqual({"agree": 47, "disagree": 1, "total": 48},
                         result["agreement"]["arms"]["A"])
        self.assertEqual({"agree": 47, "disagree": 1, "total": 48},
                         result["agreement"]["arms"]["B"])
        for axis in j.AXES:
            for arm in j.ARMS:
                agreement = result["agreement"]["per_axis"][axis][arm]
                self.assertEqual(12, agreement["agree"] + agreement["disagree"])
                self.assertEqual(12, len(agreement["items"]))
                for row in agreement["items"]:
                    self.assertEqual(row["agree"],
                                     row["cohort1_chosen_id"] == row["cohort2_chosen_id"])
        self.assertNotIn("majority", json.dumps(result))

    def test_all_ties_half_credit_balanced_positions_and_no_vote(self):
        tied = copy.deepcopy(self.scores)
        for row in tied["scores"]:
            row["values"] = {ident: 0.0 for ident in j.IDS}
        first = {r: {m["item_id"]: "FIRST" for m in self.mapping["rows"] if m["rater"] == r}
                 for r in j.RATERS}
        result = j._grade_values(self.mapping, first, tied)
        for arm in j.ARMS:
            self.assertEqual(48, result["arms"][arm]["credit"])
            self.assertEqual(96, result["arms"][arm]["exact_ties"])
            self.assertEqual(0, result["agreement"]["arms"][arm]["agree"])
        self.assertTrue(all(item["credit"] == .5 for item in result["items"]))

    def test_grade_roster_rejects_missing_duplicate_or_noncomplementary_cases(self):
        for mutate in (lambda m: m["rows"].pop(),
                       lambda m: m["rows"].__setitem__(1, m["rows"][0]),
                       lambda m: m["rows"][0].update(cohort=2),
                       lambda m: m["rows"][0].update(arm="C"),
                       lambda m: m["rows"][0].update(first_id=m["rows"][0]["second_id"])):
            bad = copy.deepcopy(self.mapping)
            mutate(bad)
            with self.assertRaises(ValueError):
                j._grade_values(bad, self.choices(), self.scores)
        noncomplementary = copy.deepcopy(self.mapping)
        for row in noncomplementary["rows"]:
            if row["cohort"] == 2:
                row["first_id"], row["second_id"] = row["second_id"], row["first_id"]
        with self.assertRaises(ValueError):
            j._grade_values(noncomplementary, self.choices(), self.scores)
        missing_choice = self.choices()
        missing_choice["rater6"].pop(next(iter(missing_choice["rater6"])))
        with self.assertRaises(ValueError):
            j._grade_values(self.mapping, missing_choice, self.scores)

    def test_new_score_schema_and_normalized_sixteen_word_stimuli_required(self):
        old = copy.deepcopy(self.scores)
        old["schema"] = "jlens_fresh_scores_v1"
        with self.assertRaises(ValueError):
            j._validate_scores(old)
        for prefix in ("fifteen words " * 7 + "last",
                       " ".join(self.data[0]["prefix"].split()[:-1]),
                       self.data[0]["prefix"].replace(" ", "  ", 1),
                       self.data[0]["prefix"] + " extra"):
            data = copy.deepcopy(self.data)
            data[0]["prefix"] = prefix
            with self.assertRaises(ValueError):
                j.make_packets(self.refs, data, self.pairs)
...
class FileStages(unittest.TestCase):
    ...
    def package(self, name="packets"):
        ...
        self.manifest = j.package(self.packet_dir, references=self.paths["references"],
            ...
            dataset_sha256=self.pins["dataset"], pairs=self.paths["pairs"],
            pairs_sha256=self.pins["pairs"], protocol=self.paths["protocol"])
    ...
    def seal(self):
        ...
        for rater in j.RATERS:
            ...
            response = {"schema": "jlens_independent_content_responses_v1", "packet_id": packet["packet_id"],
                        "responses": [{"item_id": x, "choice": "FIRST"} for x in sorted(j._public_ids(packet))]}
    ...
    def test_seal_complete_exact_bytes_and_no_score_io(self):
        ...
        self.assertEqual(288, lock["count"])
    ...
    def test_protocol_reference_and_explicit_dataset_pair_pins_are_enforced(self):
        self.pins["references"] = "0" * 64
        with self.assertRaisesRegex(ValueError, "unchanged reference"):
            self.package("wrong-reference")
        self.pins["references"] = j.FROZEN["references"]
        for name in ("dataset", "pairs"):
            saved = self.pins[name]
            self.pins[name] = "0" * 64
            with self.assertRaisesRegex(ValueError, "SHA256 mismatch"):
                self.package("wrong-" + name)
            self.pins[name] = saved
        with mock.patch.dict(j.FROZEN, {"protocol": "0" * 64}):
            with self.assertRaisesRegex(ValueError, "SHA256 mismatch"):
                self.package("wrong-protocol")

    def test_seventh_blob_must_exist_in_commit_before_any_score_read(self):
        self.seal()
        j._git(self.root, "init", "--quiet")
        # Missing rater6 while the current filesystem has all six valid responses.
        names = ["responses/lock.json", *[f"responses/{r}.json" for r in j.RATERS[:-1]]]
        j._git(self.root, "add", *names)
        j._git(self.root, "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid",
               "commit", "--quiet", "-m", "Intentionally incomplete fabricated lock")
        commit = j._git(self.root, "rev-parse", "HEAD").decode().strip()
        original, score_reads = j.read_bytes, []
        def guarded(filename, expected):
            if Path(filename) == self.paths["scores"]:
                score_reads.append(filename)
            return original(filename, expected)
        with mock.patch.object(j, "read_bytes", side_effect=guarded):
            with self.assertRaises(subprocess.CalledProcessError):
                self.grade(commit)
        self.assertEqual([], score_reads)
        self.assertFalse((self.root / "graded/grades.json").exists())

    def test_all_seven_commit_blob_checks_precede_score_access(self):
        self.seal()
        j._git(self.root, "init", "--quiet")
        j._git(self.root, "add", "responses/lock.json", *[f"responses/{r}.json" for r in j.RATERS])
        j._git(self.root, "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid",
               "commit", "--quiet", "-m", "Complete fabricated seven-blob lock")
        commit = j._git(self.root, "rev-parse", "HEAD").decode().strip()
        checked, score_reads = [], []
        ...
        def guarded(filename, expected):
            if Path(filename) == self.paths["scores"]:
                self.assertEqual({f"{commit}:responses/lock.json",
                                  *[f"{commit}:responses/{r}.json" for r in j.RATERS]}, set(checked))
                score_reads.append(filename)
            return original_read(filename, expected)
        with (mock.patch.object(j, "_git", side_effect=spy_git),
              mock.patch.object(j, "read_bytes", side_effect=guarded)):
            self.grade(commit)
        self.assertEqual(1, len(score_reads))
