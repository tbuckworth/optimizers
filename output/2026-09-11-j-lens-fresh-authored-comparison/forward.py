"""Import-inert fresh-authored-comparison token preflight and forwards only; no lens/decoding."""
...
ROOT = OUT.parents[1]
MAIN = Path('output/2026-09-11-jlens-fresh-authored-comparison')
...
PINS = {
    OUT/'raw-author.json': '8be4c015a4fcfdbaac08335dd18228a0c1f028d9bbb77fe82041d5a4699009f2',
    MAIN/'excluded-lemmas.json': '28ee8ec79b7cf8d5655d4c119fed924a548429fc24e8070727a20581d072c1d0',
    MAIN/'inputs/receipt.json': 'a24ecb04087a0aa5d7b2d40fc7b7a3e557083f6477794f6bc8dd991d599acd9d',
    MAIN/'protocol.md': 'a66353d5838df1ab5d9e0a5ff034edcda12bbd4db1a80ba6a256b4760172b71b',
...
CELLS = [(f'fa{i:02}', template) for i in range(1, 9) for template in ['active', 'passive']]
...
def pins(dataset_sha, pairs_sha):
    ...
    return {**PINS, MAIN/'inputs/dataset.json': dataset_sha, MAIN/'inputs/pairs.json': pairs_sha}
...
def finish(target, expected, outputs, extra):
    ...
    write_json(target/'receipt.json', {'schema': 'jlens_fresh_authored_'+target.name.replace('-', '_')+'_receipt_v1',
...
def load_panel():
    dataset, pairset = read_json(MAIN/'inputs/dataset.json'), read_json(MAIN/'inputs/pairs.json')
...
def token_record(row, encoded):
    ...
    require(positions['verb'] < len(ids)-1, 'verb must precede the full sentence endpoint')
...
def preflight(dataset_sha, pairs_sha, root=OUT):
    require(os.environ.get('JLENS_FRESH_AUTHORED_PREFLIGHT_RELEASE') == '1', 'parent preflight release required')
    ...
    try:
        ...
        write_json(target/'tokens.json', {'schema': 'jlens_fresh_authored_tokens_v1', 'records': records})
...
def frozen_tokens(root, receipt_sha, expected, rows):
    ...
    require(receipt['schema'] == 'jlens_fresh_authored_token_preflight_receipt_v1' and receipt['status'] == 'complete'
    ...
    require(payload['schema'] == 'jlens_fresh_authored_tokens_v1' and len(payload['records']) == 32, 'token schema/roster')
...
def score_export(h, u, mean, rows, pairs):
    ...
    export = {'schema': 'jlens_fresh_authored_scores_v1', 'locations': {
...
def forwards(dataset_sha, pairs_sha, preflight_sha, root=OUT):
    require(os.environ.get('JLENS_FRESH_AUTHORED_GPU_RELEASE') == '1', 'new parent GPU admission required')
    ...
    try:
        ...
        write_json(target/'inputs.json', {'schema': 'jlens_fresh_authored_inputs_v1', 'records': records,
