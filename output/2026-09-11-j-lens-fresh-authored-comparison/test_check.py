...
SPEC = importlib.util.spec_from_file_location('fresh_authored_check_fixture', Path(__file__).with_name('check.py'))
...
def fabricated():
    ...
    roles = ['verb']
    exports = {'scores': {'schema': 'jlens_fresh_authored_scores_v1', 'locations': {
...
class PureTests(unittest.TestCase):
    ...
    def test_gate_public_binding_and_exact_call(self):
        ...
        spec = Mock()
        with patch.object(c, 'sha', return_value=c.JUDGING_SHA), \
    ...
    def test_all_positive_and_exact_zero(self):
        ...
        self.assertTrue(result['descriptive_all_16_verb_PC4_positive'])
        ...
        self.assertFalse(result['descriptive_all_16_verb_PC4_positive'])
        self.assertEqual(1, result['summaries']['verb'][3]['zero'])
        self.assertEqual(7, result['summaries']['verb'][3]['both_templates_positive'])
        self.assertEqual(['fa01'], result['summaries']['verb'][3]['template_sign_changes'])
...
class StageTests(unittest.TestCase):
    def setUp(self):
        ...
        ctx = patch.dict(os.environ, {'JLENS_FRESH_AUTHORED_CHECK_RELEASE': '1'}); ctx.start(); self.addCleanup(ctx.stop)
    ...
    def prepare(self):
        ...
        f.write_json(target/'inputs.json', {'schema': 'jlens_fresh_authored_inputs_v1', 'records': records})
