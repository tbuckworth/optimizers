...
SPEC = importlib.util.spec_from_file_location('content_template_forward_fixture', Path(__file__).with_name('forward.py'))
...
def data():
    ...
    for ident in f.IDS:
        ...
        words = [ident]+['fabricated']*(3 if template == 'active' else 5)+['object.', 'This', 'happened', 'yesterday.']
        rows.append({'id': ident, 'pair_id': pair, 'template': template, 'pole': pole, 'text': ' '.join(words)})
    pairs = [{'id': f'{pair}-{template}', 'content_pair': pair, 'template': template,
        'observation_id': f'{pair}-{template}-O', 'provision_id': f'{pair}-{template}-P'}
        for pair, template in f.CELLS]
    return rows, pairs
...
class Tokenizer:
    ...
    def __call__(self, text, **kwargs):
        assert kwargs == {'add_special_tokens': False, 'padding': False, 'truncation': False, 'return_attention_mask': True}
...
class PureTests(unittest.TestCase):
    ...
    def test_exact_prefix_and_pair_rosters(self):
        ...
        for change in ['order', 'length', 'spacing', 'pair', 'pole', 'template', 'extra', 'suffix']:
            rr, pp = copy.deepcopy((rows, pairs))
            if change == 'order': rr.reverse()
            if change == 'length': rr[0]['text'] += ' extra'
            if change == 'spacing': rr[0]['text'] += ' '
            if change == 'pair': pp[0]['provision_id'], pp[1]['provision_id'] = pp[1]['provision_id'], pp[0]['provision_id']
            if change == 'pole': rr[0]['pole'] = 'P'
            if change == 'template': rr[0]['template'] = 'passive'
            if change == 'extra': rr.extend(copy.deepcopy(rr[:8]))
            if change == 'suffix': rr[0]['text'] = rr[0]['text'].replace('yesterday.', 'tomorrow.')
    ...
    def test_preflight_no_cutting_and_capture_postblock_last(self):
        ...
        self.assertTrue(all(r['input_ids'] == list(range(1, len(r['text'].split())+1))
            and r['attention_mask'] == [1]*len(r['text'].split())
            and r['captured_position'] == len(r['text'].split())-1 for r in records))
        model = Model(); before = f.parameter_state(model)
        h = f.capture(model, records, 'cpu')
        self.assertTrue(np.array_equal(h, np.array([[len(r['text'].split())+12]*f.WIDTH for r in rows], dtype=np.float32)))
        self.assertEqual(16, model.calls)
        self.assertTrue(torch.all(model.last_output[0, -1] == 34))
    ...
    def test_canonical_score_formula_and_bad_arrays(self):
        ...
        h = np.arange(16*f.WIDTH, dtype=np.float32).reshape(16, f.WIDTH)/32
        ...
        self.assertEqual('jlens_content_template_scores_v1', export['schema'])
    ...
    def test_gap_signs_and_zero_are_saved_score_subtractions(self):
        rows, pairs = data()
        h = np.zeros((16, f.WIDTH), dtype=np.float32)
        expected = np.array([1, -1, 0, 2, -2, 0, 3, -3], dtype=np.float64)
        h[::2, 3] = expected
        scores, gaps, _ = f.score_export(h, np.eye(f.WIDTH, 4, dtype=np.float32),
            np.zeros(f.WIDTH, dtype=np.float64), rows, pairs)
        self.assertTrue(np.array_equal(gaps, scores[::2]-scores[1::2]))
        self.assertTrue(np.array_equal(gaps[:, 3], expected))
        self.assertEqual([3, 2, 3], [int((gaps[:, 3] > 0).sum()), int((gaps[:, 3] == 0).sum()), int((gaps[:, 3] < 0).sum())])
...
class StageTests(unittest.TestCase):
    def setUp(self):
        ...
        for name, value in [('dataset', {'rows': self.rows}), ('pairs', {'pairs': self.pairs})]: f.write_json(self.root/f'{name}.json', value)
        self.dataset_sha, self.pairs_sha = [f.sha(self.root/f'{name}.json') for name in ['dataset', 'pairs']]
        for context in [patch.object(f, 'MAIN', self.root), patch.dict(f.PINS, {}, clear=True),
                        ...
                        patch.dict(os.environ, {'JLENS_CONTENT_TEMPLATE_PREFLIGHT_RELEASE': '1', 'JLENS_CONTENT_TEMPLATE_GPU_RELEASE': '1'})]:
    ...
    def test_reject_old_bare_list_and_extra_container_keys(self):
        for dataset, pairset in [(self.rows, {'pairs': self.pairs}),
                ({'rows': self.rows}, self.pairs), ({'rows': self.rows, 'extra': 1}, {'pairs': self.pairs})]:
            with patch.object(f, 'read_json', side_effect=[dataset, pairset]):
    ...
    def test_late_overlong_text_fails_whole_panel_without_partial_tokens(self):
        calls = []
        class LateBadTokenizer(Tokenizer):
            def __call__(self, text, **kwargs):
                calls.append(text)
                if len(calls) == 16: return {'input_ids': [1]*97, 'attention_mask': [1]*97}
                return super().__call__(text, **kwargs)
        with patch('transformers.AutoTokenizer.from_pretrained', return_value=LateBadTokenizer()):
            with self.assertRaises(ValueError): f.preflight(self.dataset_sha, self.pairs_sha, self.root)
    ...
    def test_complete_fake_forward_stage_without_tokenizer(self):
        ...
        with patch.dict(f.PINS, {archive: f.sha(archive)}), patch.object(f, 'OLD', self.root):
            ...
            self.assertEqual(16, receipt['forward_count'])
            self.assertEqual(16, model.calls)
            ...
            self.assertEqual('jlens_content_template_scores_v1', scores['schema'])
            self.assertTrue(all(v == (20.0 if '-active-' in ident else 22.0)
                for a in scores['scores'] for ident, v in a['values'].items()))
            gaps = f.read_json(self.root/'forwards/gaps.json')
            self.assertEqual('observation minus provision', gaps['orientation'])
            self.assertEqual(8, len(gaps['pairs']))
            self.assertTrue(all(p['gaps64'] == [0.0]*4 for p in gaps['pairs']))
