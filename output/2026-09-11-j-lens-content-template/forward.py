"""Import-inert content-template token preflight and forwards only; no lens/decoding."""
...
ROOT = OUT.parents[1]
MAIN = Path('output/2026-09-11-jlens-content-template')
...
PINS = {
    MAIN/'protocol.md': '10488950ab8234932dd1f7aa2ed74fc0d32caaef4cf5e8468d724e7206eb8c45',
...
DATASET_SHA = '841d86753f857d686165e53d76278bc1fa40840f97e09ffad61f0c89d57fcc7d'
...
def finish(target, expected, outputs, extra):
    ...
    write_json(target/'receipt.json', {'schema': 'jlens_content_template_'+target.name.replace('-', '_')+'_receipt_v1',
...
def validate_rows(rows, pairs):
    ...
    for row in rows:
        ...
        require(set(row) == {'id', 'pair_id', 'template', 'pole', 'text'}
                and (row['pair_id'], row['template'], row['pole']) == (pair, template, pole), 'row metadata')
        text = row['text']
        require(type(text) is str and len(text.split()) == (8 if template == 'active' else 10)
                and text == ' '.join(text.split()) and text.endswith('. This happened yesterday.'), 'fixed template/word-count contract')
...
def tokenize(rows, tokenizer):
    records = []
    for row in rows:
        encoded = tokenizer(row['text'], add_special_tokens=False, padding=False, truncation=False, return_attention_mask=True)
...
def preflight(dataset_sha, pairs_sha, root=OUT):
    require(os.environ.get('JLENS_CONTENT_TEMPLATE_PREFLIGHT_RELEASE') == '1', 'parent preflight release required')
    ...
    try:
        ...
        write_json(target/'tokens.json', {'schema': 'jlens_content_template_tokens_v1', 'records': records})
...
def frozen_tokens(root, receipt_sha, expected, rows):
    ...
    require(receipt['schema'] == 'jlens_content_template_token_preflight_receipt_v1' and receipt['status'] == 'complete'
    ...
    require(payload['schema'] == 'jlens_content_template_tokens_v1' and len(payload['records']) == 16, 'token schema/roster')
...
def score_export(h, u, mean, rows, pairs):
    ...
    require(h.shape == (16, WIDTH) and h.dtype == np.float32 and np.isfinite(h).all(), 'invalid saved activation shape/dtype')
    ...
    export = {'schema': 'jlens_content_template_scores_v1', 'scores': [
...
def forwards(dataset_sha, pairs_sha, preflight_sha, root=OUT):
    require(os.environ.get('JLENS_CONTENT_TEMPLATE_GPU_RELEASE') == '1', 'new parent GPU admission required')
    ...
    try:
        ...
        write_json(target/'inputs.json', {'schema': 'jlens_content_template_inputs_v1', 'records': records,
        ...
        write_json(target/'gaps.json', {'axes': [f'PC{i}' for i in range(1, 5)], 'orientation': 'observation minus provision',
        ...
        finish(target, expected, [target/n for n in ['features.npz', 'inputs.json', 'scores.json', 'gaps.json']],
            {'runtime': env, 'preflight_receipt_sha256': preflight_sha, 'model_weight_sha256': WEIGHT_SHA,
             'adapter_revision': UPSTREAM_REV, 'parameters_unchanged': True, 'forward_count': 16,
