"""Inert JSON-only saved-verb packet, response-lock and grader.

Four readers x four axes x sixteen pairs = 256 choices; arms A/C only.
Package has no score argument or key I/O. Grade validates all five committed
response-lock blobs before reading private map or score JSON. No old producer
...
CELLS = tuple((f"nv{i:02}", t) for i in range(1, 9) for t in ("active", "passive"))
...
PRODUCER_SHA = "6beef7ce564f071a854f351eba6ad6f6363deba46d7a3da15e4af6775f87b8fe"
SCORES_SHA = "431ec80549b0eea2f4b7770f28839696211247ef91275c9349e80b23b430c55b"
KEY_PINS = {
    "forward_receipt": "e985c73109909d293bbc071944fbe67c0daefbd49f9960df9e18a793eac9a07d",
    "checked_result": "d0c841c52bb9947dc45df466fdd5e0a1f9b7ba027c7eae76f1529d17cef96308",
    "checker_receipt": "1a18b75742993f43c962e386c6e9900c26aedb00b1aef14064fed25df3e49983",
}
...
def validate_inputs(references, dataset, pairs, tokens, preflight):
    keys(dataset, ("rows",))
    keys(pairs, ("pairs",))
    ...
    require(tokens["schema"] == "jlens_new_verb_transfer_tokens_v1" and
            type(tokens["records"]) is list and len(tokens["records"]) == 32, "saved token roster")
    require(type(preflight) is dict and
            preflight.get("schema") == "jlens_new_verb_transfer_token_preflight_receipt_v1" and
            preflight.get("status") == "complete" and preflight.get("source_sha256") == PRODUCER_SHA and
    ...
    require(preflight.get("outputs") == [{"path": "tokens.json", "sha256": FROZEN["tokens"]}],
            "preflight token-output binding")
    require(type(preflight.get("input_pins")) is dict and
            all(FROZEN[k] in preflight["input_pins"].values() for k in ("dataset", "pairs")),
            "preflight dataset/pair binding")
    for row, record, ident in zip(rows, tokens["records"], IDS):
        ...
        keys(record["captured_positions"], ("verb", "sentence_end"))
        require(all(type(v) is int for v in record["captured_positions"].values()) and
                record["captured_positions"] == positions and positions["verb"] < positions["sentence_end"] and
...
def make_packets(references, dataset, pairs, tokens, preflight):
    """Pure construction for fixtures and package; accepts no score/key input."""
    validate_inputs(references, dataset, pairs, tokens, preflight)
    ...
    for rater in RATERS:
        ...
        for ai, axis in enumerate(AXES):
            ...
            blocks.append(block)
        rng.shuffle(blocks)
        packets[rater] = {"schema": "jlens_verb_reader_public_v1", "packet_id": f"R{rng.getrandbits(64):016x}",
                          "prompt": PROMPT, "blocks": blocks}
    return packets, {"schema": "jlens_verb_reader_private_map_v1", "rows": private}


def _public_ids(packet):
    keys(packet, ("schema", "packet_id", "prompt", "blocks"))
    require(packet["schema"] == "jlens_verb_reader_public_v1" and packet["prompt"] == PROMPT,
...
def validate_response(response, packet):
    ...
    require(response["schema"] == "jlens_verb_reader_responses_v1" and
...
def package(output, *, references, references_sha256, dataset, dataset_sha256,
            pairs, pairs_sha256, protocol, tokens, preflight):
    attempt = Attempt(output, "package")
    try:
        require(digest(references_sha256) == FROZEN["references"], "unchanged reference pin required")
        require(digest(dataset_sha256) == FROZEN["dataset"] and digest(pairs_sha256) == FROZEN["pairs"],
                "frozen dataset/pair pins required")
        ...
        token_data, kr = read_json(tokens, FROZEN["tokens"])
        preflight_data, fr = read_json(preflight, FROZEN["preflight"])
        packets, mapping = make_packets(refs, data, pair_data, token_data, preflight_data)
        public = {r: attempt.write(f"public/{r}.json", packets[r]) for r in RATERS}
        private = attempt.write("private-map.json", mapping)
        return attempt.write("manifest.json", {"schema": "jlens_verb_reader_packets_v1", "seed": SEED,
            "python": sys.version, "source_sha256": source_sha(),
            "inputs": {"references": rr, "dataset": dr, "pairs": pr, "protocol": tr, "tokens": kr, "preflight": fr},
...
def _bundle(directory, expected):
    ...
    require(manifest["schema"] == "jlens_verb_reader_packets_v1" and type(manifest["seed"]) is int and
...
def seal_responses(output, *, packets, manifest_sha256, responses):
    ...
    try:
        ...
        return attempt.write("lock.json", {"schema": "jlens_verb_reader_response_lock_v1",
...
def _validate_scores(value):
    ...
    require(value["schema"] == "jlens_new_verb_transfer_scores_v1", "score schema")
    keys(value["locations"], ("verb", "sentence_end"))
    require(list(value["locations"]) == ["verb", "sentence_end"], "score location order")
    out = {}
...
def _grade_roster(mapping, choices):
    ...
    require(mapping["schema"] == "jlens_verb_reader_private_map_v1" and
...
def _grade_values(mapping, choices, score_export):
    ...
    return {"schema": "jlens_verb_reader_grades_v1", "primary": primary,
...
def grade(output, *, packets, manifest_sha256, responses, lock_sha256,
    ...
    try:
        ...
        require(lock["schema"] == "jlens_verb_reader_response_lock_v1" and type(lock["count"]) is int and
        ...
        expected_public, expected_map = make_packets(loaded["references"], loaded["dataset"], loaded["pairs"],
                                                     loaded["tokens"], loaded["preflight"])
        ...
        require(digest(scores_sha256) == SCORES_SHA, "frozen verb-score pin required")
        ...
        result["provenance"] = {"source_sha256": source_sha(), "packet_manifest_sha256": manifest_sha256,
                                ...
                                "key_provenance_pins": KEY_PINS, "graded_location": "verb"}
