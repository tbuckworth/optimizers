"""Import-inert endpoint-location token preflight and forwards only; no lens/decoding."""
...
ROOT = OUT.parents[1]
MAIN = Path('output/2026-09-11-jlens-endpoint-location')
OLD = ROOT/'output/2026-09-10-j-lens-fresh-content'
PRIOR = ROOT/'output/2026-09-11-j-lens-content-template'
LOCATIONS = ['verb', 'sentence_end']
SUFFIX = ' This happened yesterday.'
...
PINS = {
    MAIN/'proposed-design.md': 'c1d1aa9d988a78881a3c654ae80dfe2ca4955a897e238ac4e13168a7f249dade',
    PRIOR/'forward.py': '9a3b2179b7b613e5561f8cd95b4cb1a4e8683bc8f5c41c98bcd5ff943fddcd03',
    PRIOR/'token-preflight/tokens.json': 'c276357f0b89efa3c8a2f54d836d852e24535053401c1af516338a9b2207f86f',  # gitleaks:allow -- independently verified scientific token-ID artifact SHA256, not a credential
    PRIOR/'token-preflight/receipt.json': 'ff398d5a0816cedb6566a867ea4d036666db3b60777a57a23c9aea5c5bf366ab',
    MAIN/'protocol.md': '6095b51f81993be06210c81f32d621b00640130ec77da783e1c774e4566cded3',
...
CELLS = [(f'{i:02}', template) for i in range(1, 5) for template in ['active', 'passive']]
IDS = [f'{pair}-{template}-{pole}' for pair, template in CELLS for pole in ['O', 'P']]
LAYER, WIDTH, VOCAB = 11, 1024, 248320
DATASET_SHA = 'bda6dbefe07ebfa62cbf2efc483b6164cc819debfd1d124460e5153bdd2b7ee8'
PAIRS_SHA = 'd47d27b93e48b38deb5e4871596c906057ba46e18305e3849b02ad1b0e337ac0'
...
def pins(dataset_sha, pairs_sha):
    ...
    require((dataset_sha, pairs_sha) == (DATASET_SHA, PAIRS_SHA), 'frozen design pins required')
    return {**PINS, MAIN/'dataset.json': dataset_sha, MAIN/'pairs.json': pairs_sha}
...
def finish(target, expected, outputs, extra):
    ...
    write_json(target/'receipt.json', {'schema': 'jlens_endpoint_location_'+target.name.replace('-', '_')+'_receipt_v1',
...
def validate_rows(rows, pairs):
    require(type(rows) is list and len(rows) == 16 and all(type(r) is dict for r in rows), 'exact 16-row roster required')
    require([r.get('id') for r in rows] == IDS, 'exact 16-row ordering required')
    for row in rows:
        pair, template, pole = row['id'].split('-')
        require(set(row) == {'id', 'pair_id', 'template', 'pole', 'text', 'verb', 'verb_span'}
                and (row['pair_id'], row['template'], row['pole']) == (pair, template, pole), 'row metadata')
        text = row['text']
        require(type(text) is str and len(text.split()) == (5 if template == 'active' else 7)
                and text == ' '.join(text.split()) and text.endswith('.'), 'fixed template/word-count contract')
        span, verb = row['verb_span'], row['verb']
        require(type(span) is list and len(span) == 2 and all(type(x) is int for x in span)
                and 0 <= span[0] < span[1] < len(text) and type(verb) is str
                and verb.isalpha() and text[slice(*span)] == verb, 'exact verb span')
    require(len({r['text'] for r in rows}) == 16, 'duplicate text')
    require(pairs == [{'id': f'{pair}-{template}', 'content_pair': pair, 'template': template,
        'observation_id': f'{pair}-{template}-O', 'provision_id': f'{pair}-{template}-P'}
        for pair, template in CELLS], 'exact eight observation/provision pairs required')


def load_panel():
    dataset, pairset = read_json(MAIN/'dataset.json'), read_json(MAIN/'pairs.json')
    require(type(dataset) is dict and set(dataset) == {'rows'}
            and type(pairset) is dict and set(pairset) == {'pairs'}, 'exact panel containers required')
    rows, pairs = dataset['rows'], pairset['pairs']
...
def load_prior(rows):
    payload = read_json(PRIOR/'token-preflight/tokens.json')
    receipt = read_json(PRIOR/'token-preflight/receipt.json')
    require(receipt['schema'] == 'jlens_content_template_token_preflight_receipt_v1'
            and receipt['status'] == 'complete'
            and receipt['source_sha256'] == PINS[PRIOR/'forward.py']
            and receipt['outputs'] == [{'path': 'tokens.json', 'sha256': PINS[PRIOR/'token-preflight/tokens.json']}], 'prior receipt binding')
    require(payload['schema'] == 'jlens_content_template_tokens_v1'
            and len(payload['records']) == 16, 'prior token schema/roster')
    for old, row in zip(payload['records'], rows, strict=True):
        expected = {k: v for k, v in row.items() if k not in {'verb', 'verb_span'}}
        expected['text'] += SUFFIX
        require({k: old[k] for k in expected} == expected, 'exact prior suffix/row identity')
        validate_ids(old['input_ids'], old['attention_mask'])
        require(old['captured_position'] == len(old['input_ids'])-1 and old['layer'] == LAYER, 'prior capture contract')
...
def token_record(row, encoded, prior):
    ids, mask, offsets = encoded['input_ids'], encoded['attention_mask'], encoded['offset_mapping']
    validate_ids(ids, mask)
    require(len(ids) < len(prior['input_ids']) and ids == prior['input_ids'][:len(ids)], 'shared token prefix drift')
    text = row['text']
    ...
    require(all(offsets[i][1] <= offsets[i+1][0] for i in range(len(offsets)-1)), 'overlapping/unordered offsets')
    start, end = row['verb_span']
    overlap = [i for i, (a, b) in enumerate(offsets) if a < end and b > start]
    require(overlap and overlap == list(range(overlap[0], overlap[-1]+1)), 'noncontiguous verb tokens')
    first, last = offsets[overlap[0]], offsets[overlap[-1]]
    require(first[0] <= start and (first[0] == start or text[first[0]:start].isspace())
            and last[1] == end and all(offsets[i][1] == offsets[i+1][0] for i in overlap[:-1]), 'incomplete verb coverage or merged boundary')
    require(offsets[-1][0] <= len(text)-1 and offsets[-1][1] == len(text)
            and text[-1] == '.', 'final period coverage')
    positions = {'verb': overlap[-1], 'sentence_end': len(ids)-1}
    require(positions['verb'] < positions['sentence_end'], 'distinct ordered positions')
    return {**row, 'input_ids': ids, 'attention_mask': mask, 'offset_mapping': offsets,
            'captured_positions': positions,
            'selected_substrings': {k: text[slice(*offsets[v])] for k, v in positions.items()}, 'layer': LAYER}


def tokenize(rows, tokenizer, prior):
    records = []
    for row, old in zip(rows, prior, strict=True):
        ...
        records.append(token_record(row, encoded, old))
    return records


def preflight(dataset_sha, pairs_sha, root=OUT):
    require(os.environ.get('JLENS_ENDPOINT_LOCATION_PREFLIGHT_RELEASE') == '1', 'parent preflight release required')
    ...
    try:
        ...
        records = tokenize(rows, tokenizer, load_prior(rows))
        write_json(target/'tokens.json', {'schema': 'jlens_endpoint_location_tokens_v1', 'records': records})
        finish(target, expected, [target/'tokens.json'], {'runtime': env, 'count': 16, 'elapsed_seconds': time.monotonic()-started,
...
def frozen_tokens(root, receipt_sha, expected, rows):
    ...
    require(receipt['schema'] == 'jlens_endpoint_location_token_preflight_receipt_v1' and receipt['status'] == 'complete'
    ...
    require(payload['schema'] == 'jlens_endpoint_location_tokens_v1' and len(payload['records']) == 16, 'token schema/roster')
    prior = load_prior(rows)
    for record, row, old in zip(payload['records'], rows, prior, strict=True):
        require(record == token_record(row, record, old), 'frozen token/position identity')
...
def score_export(h, u, mean, rows, pairs):
    ...
    require(h.shape == (16, 2, WIDTH) and h.dtype == np.float32 and np.isfinite(h).all(), 'invalid saved activation shape/dtype')
    scores = (h.astype(np.float64)-mean) @ u.astype(np.float64)
    index = {r['id']: i for i, r in enumerate(rows)}
    gaps = np.stack([scores[index[p['observation_id']]]-scores[index[p['provision_id']]] for p in pairs])
    require(np.isfinite(scores).all() and np.isfinite(gaps).all(), 'nonfinite geometric outputs')
    export = {'schema': 'jlens_endpoint_location_scores_v1', 'locations': {
...
def forwards(dataset_sha, pairs_sha, preflight_sha, root=OUT):
    require(os.environ.get('JLENS_ENDPOINT_LOCATION_GPU_RELEASE') == '1', 'new parent GPU admission required')
    ...
    try:
        ...
        write_json(target/'inputs.json', {'schema': 'jlens_endpoint_location_inputs_v1', 'records': records,
        ...
        write_json(target/'gaps.json', {'locations': LOCATIONS, 'axes': [f'PC{i}' for i in range(1, 5)], 'orientation': 'observation minus provision',
        ...
        finish(target, expected, [target/n for n in ['features.npz', 'inputs.json', 'scores.json', 'gaps.json']],
            {'runtime': env, 'preflight_receipt_sha256': preflight_sha, 'model_weight_sha256': WEIGHT_SHA,
             'adapter_revision': UPSTREAM_REV, 'parameters_unchanged': True, 'forward_count': 16, 'capture_locations': LOCATIONS,
