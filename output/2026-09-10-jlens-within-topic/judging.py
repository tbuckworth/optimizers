...
Responses have exactly {"schema":"jlens_within_topic_responses_v1","packet_id":...,
...
PROMPT = ("Using only the two reference descriptions, which of the two prefixes "
          ...
          "even if uncertain.")
FROZEN = {
    "protocol": "61dde1c009b33daa88148859268606e11b70da92215061c95a9a25364553b559",
    "dataset": "c655a5531af3215a29e332ddc6b23ac4854b0bffdf91b8ad5e8a045748ff7cd9",
    "pairs": "552b4090c2e2382a352a14d2b6be3bd28cad98a33f8f0c36dd49dde0a89fad7a",
}
MAX_JSON = 1024 * 1024
...
def make_packets(references, dataset, pairs):
    ...
    for rater in RATERS:
        ...
        for ai, axis in enumerate(AXES):
            ...
            blocks.append(block)
        rng.shuffle(blocks)
        packets[rater] = {"schema": "jlens_within_topic_public_v1", "packet_id": f"R{rng.getrandbits(64):016x}",
                          "prompt": PROMPT, "blocks": blocks}
    return packets, {"schema": "jlens_within_topic_private_map_v1", "rows": private}


def _public_ids(packet):
    keys(packet, ("schema", "packet_id", "prompt", "blocks"))
    require(packet["schema"] == "jlens_within_topic_public_v1" and packet["prompt"] == PROMPT,
...
def validate_response(response, packet):
    ...
    require(response["schema"] == "jlens_within_topic_responses_v1" and
...
def package(output, *, references, references_sha256, dataset, pairs, protocol):
    ...
    try:
        ...
        return attempt.write("manifest.json", {"schema": "jlens_within_topic_packets_v1", "seed": SEED,
...
def _bundle(directory, expected):
    ...
    require(manifest["schema"] == "jlens_within_topic_packets_v1" and type(manifest["seed"]) is int and
...
def seal_responses(output, *, packets, manifest_sha256, responses):
    ...
    try:
        ...
        return attempt.write("lock.json", {"schema": "jlens_within_topic_response_lock_v1",
...
def _grade_values(mapping, choices, score_export):
    ...
    return {"schema": "jlens_within_topic_grades_v1", "arms": arms, "per_axis": per_axis,
...
def grade(output, *, packets, manifest_sha256, responses, lock_sha256,
    ...
    try:
        ...
        require(lock["schema"] == "jlens_within_topic_response_lock_v1" and type(lock["count"]) is int and
