...
SPEC = importlib.util.spec_from_file_location('new_verb_fixture', Path(__file__).with_name('forward.py'))
...
class PureTests(unittest.TestCase):
    ...
    def test_two_postblock_positions_one_forward_no_mutation(self):
        ...
        self.assertEqual((32, 2, f.WIDTH), h.shape)
    ...
    def test_scores_gap_order_and_export(self):
        ...
        h = np.arange(32*2*f.WIDTH, dtype=np.float32).reshape(32, 2, f.WIDTH)/32
        ...
        self.assertEqual((16, 2, 4), gaps.shape); self.assertEqual(np.float64, scores.dtype)
        self.assertEqual(f.LOCATIONS, list(export['locations']))
        self.assertEqual(f.IDS, list(export['locations']['verb'][0]['values']))
        self.assertEqual('jlens_new_verb_transfer_scores_v1', export['schema'])
...
class StageTests(unittest.TestCase):
    def setUp(self):
        ...
        for ctx in [patch.object(f, 'MAIN', self.root), patch.dict(f.PINS, {}, clear=True),
                    ...
                    patch.dict(os.environ, {'JLENS_NEW_VERB_TRANSFER_PREFLIGHT_RELEASE': '1', 'JLENS_NEW_VERB_TRANSFER_GPU_RELEASE': '1'})]:
    ...
    def test_fake_end_to_end_no_forward_tokenizer(self):
        ...
        with patch.dict(f.PINS, {archive: f.sha(archive)}), patch.object(f, 'OLD', self.root):
            ...
            with np.load(self.root/'forwards/features.npz', allow_pickle=False) as saved:
                ...
                self.assertEqual((32, 2, f.WIDTH), saved['activation_11'].shape)
                self.assertEqual((32, 2, 4), saved['scores64'].shape)
                self.assertEqual((16, 2, 4), saved['gaps64'].shape)
