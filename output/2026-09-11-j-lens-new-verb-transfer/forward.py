"""Import-inert new-verb-transfer token preflight and forwards only; no lens/decoding."""
...
ROOT = OUT.parents[1]
MAIN = Path('output/2026-09-11-jlens-new-verb-transfer')
...
PINS = {
    OUT/'proposed-roster.md': '8396ce372fdd752c551bde763ca6a97f7cabe0c13eff54e9773914dd5e75bbad',
    MAIN/'protocol.md': '8ab3c713b7b4a34aa6508a5c72ef0985453f542ac56f6a09209ac8c98a2d8e85',
...
CELLS = [(f'nv{i:02}', template) for i in range(1, 9) for template in ['active', 'passive']]
IDS = [f'{pair}-{template}-{pole}' for pair, template in CELLS for pole in ['O', 'P']]
LAYER, WIDTH, VOCAB = 11, 1024, 248320
DATASET_SHA = 'fd4605ab7a54ad17dd6aed27492f1fee61f6924c5515583c0f911b547d484d00'
PAIRS_SHA = '735b9b88c19654fa38251cee0221dc65dd523feb24864dce139a25b390f683c7'
...
def finish(target, expected, outputs, extra):
    ...
    write_json(target/'receipt.json', {'schema': 'jlens_new_verb_transfer_'+target.name.replace('-', '_')+'_receipt_v1',
...
def validate_rows(rows, pairs):
    ...
    for row in rows:
        ...
        require(type(text) is str and len(text.split()) == (5 if template == 'active' else 7)
                and text.isascii() and text == ' '.join(text.split()) and text.endswith('.'), 'fixed template/word-count contract')
    ...
    require(len({r['text'] for r in rows}) == 32, 'duplicate text')
    require(pairs == [{'id': f'{pair}-{template}', 'content_pair': pair, 'template': template,
        ...
        for pair, template in CELLS], 'exact sixteen observation/provision pairs required')
...
def preflight(dataset_sha, pairs_sha, root=OUT):
    require(os.environ.get('JLENS_NEW_VERB_TRANSFER_PREFLIGHT_RELEASE') == '1', 'parent preflight release required')
    ...
    try:
        ...
        write_json(target/'tokens.json', {'schema': 'jlens_new_verb_transfer_tokens_v1', 'records': records})
...
def frozen_tokens(root, receipt_sha, expected, rows):
    ...
    require(receipt['schema'] == 'jlens_new_verb_transfer_token_preflight_receipt_v1' and receipt['status'] == 'complete'
    ...
    require(payload['schema'] == 'jlens_new_verb_transfer_tokens_v1' and len(payload['records']) == 32, 'token schema/roster')
...
def score_export(h, u, mean, rows, pairs):
    ...
    require(h.shape == (32, 2, WIDTH) and h.dtype == np.float32 and np.isfinite(h).all(), 'invalid saved activation shape/dtype')
    ...
    export = {'schema': 'jlens_new_verb_transfer_scores_v1', 'locations': {
...
def forwards(dataset_sha, pairs_sha, preflight_sha, root=OUT):
    require(os.environ.get('JLENS_NEW_VERB_TRANSFER_GPU_RELEASE') == '1', 'new parent GPU admission required')
    ...
    try:
        ...
        write_json(target/'inputs.json', {'schema': 'jlens_new_verb_transfer_inputs_v1', 'records': records,
