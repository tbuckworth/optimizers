#!/usr/bin/env python3
"""Losslessly collect every I15 scalar JSON after a hash-pinned passing audit."""
...
AUDIT_SCHEMA = "i15_history_analysis_audit_v1"
COLLECTION_SCHEMA = "i15_scalar_collection_v1"
...
def _preflight(source: Path, audit_path: Path, audit_sha256: str):
    ...
    if actual_root != expected_root:
        raise ValueError("Source root membership differs from the I15 protocol")
    for phase in PHASES:
        path = source / phase
    ...
    for phase in PHASES:
        ...
        if type(completion) is not dict \
                or completion.get("schema") != "i15_completion_v1" \
            ...
            raise ValueError("I15 phase completion or numerical-failure accounting differs")
        records = completion.get("artifacts")
        if type(records) is not list:
            raise ValueError("I15 completion lacks an artifact list")
        manifest = {}
        for record in records:
            name, size, digest = _artifact_record(record)
            if name in manifest:
                raise ValueError("I15 completion contains duplicate artifact names")
        ...
        if type(attempt) is not dict \
                or attempt.get("schema") != "i15_attempt_v1" \
    ...
    def write(self, raw):
        if len(raw) > self.remaining:
            if self.remaining:
                ...
                self.remaining = 0
            raise RuntimeError("I15 scalar collection exceeds the 512 MiB output cap")
...
def collect(root, output, audit, audit_sha256):
    ...
    manifest = {"schema": COLLECTION_SCHEMA, "status": "complete",
        ...
        "scope": ("Every original I15 JSON artifact, both phase completions, and both root "
                  "attempt records; runtime entries and I14 references remain hash-bound but "
                  "are not archived."),
        "json_count": len(rows),
