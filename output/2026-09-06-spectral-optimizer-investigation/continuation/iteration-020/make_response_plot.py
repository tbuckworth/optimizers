...
def _render_mse(
    ...
    figure.text(
        0.5, 0.018,
        "Eight-policy pre-outcome subset; CSV retains all 864 new and 472 reused-parent mean rows.\n"
        "The useful-direction oracle is privileged; this is not fresh confirmation.",
...
def _render_alignment(path: Path, direction_index: dict, plt) -> list[dict]:
    ...
    figure.text(
        0.5, 0.02,
        "Alignment is a direction diagnostic, not a plug-in prediction of native performance.\n"
        "These are reused seeds, not fresh confirmation.",
        ha="center", va="bottom", fontsize=9,
    )
    figure.subplots_adjust(left=0.08, right=0.985, bottom=0.21, top=0.79, wspace=0.25)
    with path.open("xb") as handle:
        figure.savefig(handle, format="png", dpi=150, facecolor="white")
    plt.close(figure)
    return plotted
...
def render(
    ...
    if output_directory.name != "plots-002" \
            or output_directory.parent.resolve(strict=True) != HERE \
            or output_directory.exists() or output_directory.is_symlink():
        raise PlotError("output must be the absent exclusive iteration-020/plots-002")
